<template>
  <div class="onjob-training-question-bank-list">
    ...
    <div class="question-list">
      <div class="list-header">
        ...
        <div class="filter-controls">
          ...
          <select v-model="courseFilter" @change="filterQuestions">
...
import { 
  getOnJobTrainingQuestionsByUser, 
  deleteOnJobTrainingQuestion,
  getCourseOptions 
...
const filteredQuestions = computed(() => {
  ...
  if (courseFilter.value) {
    filtered = filtered.filter(q => q.courseId === courseFilter.value)
  }
...
function loadQuestions() {
  questions.value = getOnJobTrainingQuestionsByUser(props.currentUser.id)
}
...
function deleteQuestion(id) {
  if (confirm('确定要删除这道题目吗？删除后无法恢复。')) {
    if (deleteOnJobTrainingQuestion(id)) {
      loadQuestions()
      alert('题目删除成功！')
    } else {
      alert('删除失败，请重试！')
...
<style scoped>
.onjob-training-question-bank-list {
  padding: 20px;
  max-width: 1600px;
  margin: 0 auto;
}
