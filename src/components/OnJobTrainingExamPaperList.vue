<template>
  <div class="onjob-training-exam-paper-list">
    ...
    <div class="paper-list">
      <div class="list-header">
        ...
        <div class="filter-controls">
          ...
          <select v-model="courseFilter" @change="filterPapers">
...
import { 
  getOnJobTrainingExamPapersByUser, 
  deleteOnJobTrainingExamPaper, 
  getCourseOptions 
...
const filteredPapers = computed(() => {
  ...
  if (courseFilter.value) {
    filtered = filtered.filter(p => p.courseId === courseFilter.value)
  }
...
// 加载试卷列表
function loadPapers() {
  papers.value = getOnJobTrainingExamPapersByUser(props.currentUser.id)
}
...
function deletePaper(id) {
  if (confirm('确定要删除这份试卷吗？删除后无法恢复。')) {
    if (deleteOnJobTrainingExamPaper(id)) {
      loadPapers()
      alert('试卷删除成功！')
    } else {
      alert('删除失败，请重试！')
...
<style scoped>
.onjob-training-exam-paper-list {
  padding: 20px;
  max-width: 1600px;
  margin: 0 auto;
}
