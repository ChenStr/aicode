        ...
        <form @submit.prevent="savePaper">
          ...
          <div class="form-section">
            ...
            <div class="form-row">
              ...
              <div class="form-group">
                <label>适用MTA *</label>
                <select v-model="formData.mtaAuthId" required class="form-select">
                  <option value="">请选择适用MTA</option>
                  <option 
...
const totalPoints = computed(() => {
  return calculateTotalPoints(formData.value.questionSettings)
})

const canSave = computed(() => {
  return formData.value.name.trim() && 
         formData.value.mtaAuthId && 
...
function loadPaperData() {
  if (!isNew.value && props.paperId) {
    const paper = getExamPaperById(props.paperId)
    if (paper) {
      formData.value = {
        name: paper.name,
        description: paper.description || '',
        mtaAuthId: paper.mtaAuthId,
        status: paper.status,
...
function getAvailableQuestions(typeKey, diffKey) {
  if (!formData.value.mtaAuthId) return 0
  ...
  const questions = getQuestionsByUser(props.currentUser.id)
  return questions.filter(q => 
    q.type === typeMap[typeKey] && 
    q.difficulty === difficultyMap[diffKey] &&
    q.mtaAuthId === formData.value.mtaAuthId
  ).length
}
...
function savePaper() {
  ...
  try {
    if (isNew.value) {
      const id = createExamPaper(paperData)
      alert('试卷创建成功！')
    } else {
      const success = updateExamPaper(props.paperId, paperData)
      if (success) {
        alert('试卷更新成功！')
      } else {
        alert('更新失败，请重试！')
