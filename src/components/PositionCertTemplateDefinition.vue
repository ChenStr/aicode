...
import { positionCertTemplatesStore, listTemplatesByDepartment, createTemplate, updateTemplate, removeTemplate } from '../positionCertTemplates'
...
const filtered = computed(() => {
  ...
  return visible.value.filter(t => {
    return !kw || t.name.toLowerCase().includes(kw) || (t.description||'').toLowerCase().includes(kw) || getPositionName(t.positionId).toLowerCase().includes(kw)
  })
})
...
const form = ref({ id: '', department: '', positionId: '', name: '', description: '', backgroundUrl: '', defaultExpireYears: 5, contentTemplate: '', variables: [] })

function openCreate() {
  editing.value = null
  form.value = { id: '', department: props.currentUser.department, positionId: '', name: '', description: '', backgroundUrl: '', defaultExpireYears: 5, contentTemplate: '【岗位证书】\n{{applicantName}} 同志已达到岗位“{{positionName}}”的培训与考核要求。\n签发日期：{{issuedAt}}  有效期至：{{expireAt}}', variables: [
...
function saveEdit() {
  if (!form.value.positionId) { ElMessage.error('请选择关联岗位'); return }
...
<template>
  <section class="cert-template-page">
    ...
    <div class="filters">
      <el-form inline>
        <el-form-item label="关键字">
          <el-input v-model="keyword" placeholder="模版名称/描述/岗位" clearable style="width: 260px" />
        </el-form-item>
      </el-form>
    </div>
    ...
    <el-table :data="paged" stripe>
      ...
      <el-table-column label="关联岗位" min-width="220">
    ...
    <el-dialog v-model="editDialog" :title="editing ? '编辑模版' : '新增模版'" width="760px">
      <el-form label-width="120px">
        <el-form-item label="关联岗位">
          <el-select v-model="form.positionId" filterable placeholder="选择所属岗位" style="width: 100%">
            <el-option v-for="p in listPositionsByDept(props.currentUser.department)" :key="p.id" :label="p.name" :value="p.id" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="内容模版">
          <el-input v-model="form.contentTemplate" type="textarea" :rows="8" placeholder="可在文本中使用 {{applicantName}}、{{issuedAt}}、{{expireAt}}、{{positionName}} 等变量" />
        </el-form-item>
